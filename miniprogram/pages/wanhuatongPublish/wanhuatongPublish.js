...
Page({
  ...
  async onSubmit(e) { // 1. 不存在空项 2. 结束时间 > 开始时间
    try {
      ...
      //将可能出现的违规内容（联系方式、简介、名称、详细地点）全都放在一起检测
      let content = value.contact
      let contentintroduction = value.introduction
      let contentname = value.name
      let contentcontact = value.contact
      let contentplaceDetail = value.placeDetail
      content = contentintroduction.concat(contentname)
      content = content.concat(contentcontact)
      content = content.concat(contentplaceDetail)
